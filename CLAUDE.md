...
- **v1.4.0 (in progress)**: Real native data layer + ORM — resolves issue #5 ("how do I use the database pool and Redis client?"). The previous DB/Redis layer was **mock scaffolding** (methods returned `[]`/`None`, never connected); it is now real.
  ...
  - **Known follow-up**: TLS for Redis; `numeric`/`timestamptz` params need an explicit `$1::type` cast; `cargo test --lib` can't link libpython (pre-existing pyo3 `extension-module` limitation) — Python integration tests are the verification path.
