...
description: Learn how to implement Event Sourcing, CQRS (Command Query Responsibility Segregation), and Saga patterns with compensations in Cello
---

# :material-source-branch: Advanced Architectural Patterns

This example demonstrates how to implement three advanced architectural patterns in Cello: Event Sourcing, CQRS (Command Query Responsibility Segregation), and Sagas for managing distributed transactions with automated compensation flows.

## Features Demonstrated

- **Event Sourcing**: Restoring domain state by replaying aggregate events retrieved from an `EventStore`.
- **CQRS command/query splitting**: Route writes through a `CommandBus` and reads through a `QueryBus` for fast read scaling.
- **Saga Orchestrations**: Coordinating multi-step processes (inventory reserve -> billing -> order creation) with reversed compensation rollbacks if a step fails.
- **Lifecycle Startup Hooks**: Running async initializations (connecting database event streams) at application startup.
