...
description: Deploy Cello with production-grade cluster workers, HTTP/2, HTTP/3, TLS settings, static asset configurations, timeouts, and headers
---

# :material-domain: Enterprise Configuration

This example demonstrates how to configure production-grade settings in Cello, covering timeouts, payload sizes, multi-worker process clustering, TLS certifications, HTTP/2 and HTTP/3 protocol configurations, static files, and security contexts.

## Features Demonstrated

- **Multi-Worker Process Clusters**: Spawning multiple process instances using `ClusterConfig` with CPU affinity pinning.
- **Resource Constraints**: Configuring connection capacities, header parameters, and request limits.
- **TLS & HTTP Upgrades**: Mapping secure paths, TLS protocols, and parameters for HTTP/2 and HTTP/3 multiplexing.
- **Dynamic Policy Configuration**: Structuring timeout behaviors, cookies, static directories, and rate limit algorithms.
...
curl http://127.0.0.1:8000/config/timeout
curl http://127.0.0.1:8000/config/cluster
```

## Key Concepts

- **Clustered Workers**: Spawning worker processes with `ClusterConfig(workers=4)` helps maximize CPU cores when running CPU-heavy operations.
- **Protocol Configuration**: Using `Http2Config` or `Http3Config` with `TlsConfig` activates modern network upgrades on top of standard sockets.
- **Timeout Tuning**: Configuring header, body, handler, write, and idle timeouts protects servers from Slowloris DDoS and hung threads.
