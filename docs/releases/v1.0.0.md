...
description: Cellon v1.0.0 - Production-Ready Stable Release
---
...
**Multi-Process Workers (SO_REUSEPORT):** Cello uses `os.fork()` to spawn N+1 worker processes (N children + parent), each binding to the same port via `SO_REUSEPORT`. The Linux kernel distributes connections across workers, bypassing the Python GIL bottleneck and achieving near-linear scaling with core count.
